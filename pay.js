// pay.js — หน้าชำระเงิน: เลือกเดือน แสดงสถานะการจ่ายเงิน
let selectedId = null;
let searchQuery = "";
...
async function loadLatestMembers() {
    try {
        ...
        if (response.ok) {
            ...
            if (Array.isArray(data) && data.length > 0) {
                console.log(`✅ โหลดข้อมูลสมาชิก ${data.length} คนสำเร็จ`);
                return data;
            }
        } else {
    ...
    console.warn("⚠️ ไม่สามารถโหลดข้อมูล ส่งคืนรายชื่อว่าง");
    return [];
}

async function initApp() {
    console.log("🚀 initApp() เริ่มทำงาน");
    ...
    if (backTo && MEMBERS.some(m => m.id === Number(backTo))) {
        console.log(`👤 แสดงโปรไฟล์สมาชิก ID: ${backTo}`);
...
// เรียก initApp เมื่อ DOM loaded เสร็จ
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initApp);
} else {
    // ถ้า DOM loaded เสร็จแล้ว เรียกทันที
    initApp();
}
